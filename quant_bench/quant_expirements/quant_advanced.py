...
def make_rotation(n: int, device, dtype=torch.float32, kind: str = "hadamard"):
    ...
    if kind == "dct":
        import rotations  # same dir; orthonormal DCT-II, valid for any n (incl. non-pow2 like 11008)
        return rotations._dct(n).to(device, dtype)
...
class RotatedQuantLinear(nn.Module):
    ...
    def __init__(self, linear: nn.Linear, Q: torch.Tensor, w_bits: int,
                 a_bits: int | None = None, Q_buffer: torch.Tensor | None = None):
        super().__init__()
        ...
        # Q is identical for every linear with the same in_features. Share ONE buffer
        # across them (pass Q_buffer) instead of storing a per-instance copy — the copies
        # otherwise dominate memory (~14 GB for Llama-7B, e.g. 11008² down_proj rotations).
        if Q_buffer is None:
            Q_buffer = Q.to(dtype)
...
class Rotation:
    ...
    def apply(self, model, ctx=None):
        ...
        mdtype = model.language_model.model.embed_tokens.weight.dtype
        rotations: dict[int, torch.Tensor] = {}        # fp32, for the weight rotation math
        rot_buffers: dict[int, torch.Tensor] = {}      # shared low-precision Q used at forward
        items = list(quant.backbone_linears(model))
        n = 0
        for i in range(len(items)):
            full, parent, attr, lin = items[i]
            d = lin.in_features
            if d not in rotations:
                rotations[d] = make_rotation(d, dev, torch.float32, self.kind)
                rot_buffers[d] = rotations[d].to(mdtype)
            setattr(parent, attr,
                    RotatedQuantLinear(lin, rotations[d], self.w_bits, self.a_bits,
                                       Q_buffer=rot_buffers[d]).to(dev))
            # Drop the reference to the original bf16 linear so its weight can be freed
            # immediately instead of staying resident until the whole loop finishes.
            items[i] = None
            del lin
            n += 1
        if dev.type == "cuda":
            torch.cuda.empty_cache()
        kinds = {d: (self.kind if (self.kind != "hadamard" or _is_pow2(d)) else "orth(fallback)")
                 for d in rotations}
