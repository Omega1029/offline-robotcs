...
OFT_CHUNK_SIZE = 8
OFT_ACTION_DIM = 7
# Standard OpenVLA-OFT eval protocol: per-suite max steps (NOT the old flat 600).
TASK_MAX_STEPS = {
    "libero_spatial": 220, "libero_object": 280, "libero_goal": 300,
    "libero_long": 520, "libero_10": 520, "libero_90": 400,
}
MAX_STEPS_PER_ROLLOUT = 600  # fallback only
...
def run_rollout(
    ...
    cap = max_steps + NUM_STEPS_WAIT  # OFT convention: NUM_STEPS_WAIT settle + max_steps active
    ...
    for task_idx, task_name in enumerate(tqdm(task_names, desc=f"{suite_name} tasks")):
        ...
        for rollout_idx in range(rollouts_per_task):
            ...
            result = run_rollout(
                ...
                max_steps=TASK_MAX_STEPS.get(suite_name, MAX_STEPS_PER_ROLLOUT),
            )
            result.update({
                "suite": suite_name,
                "task": task_name,
...
def parse_args():
    ...
    p.add_argument("--quant_bits", type=int, default=0,
                   help=">0 and <16: fake-quantize the language_model backbone to this many bits "
                        "(weight-only) before eval. 0=bf16.")
    p.add_argument("--rotation", type=str, default="none",
                   choices=["none", "dct", "hadamard", "random_orthogonal"],
                   help="With --quant_bits>0: apply QuaRot/SpinQuant-style incoherence rotation "
                        "before quant. 'dct'=orthonormal DCT-II.")
    return p.parse_args()
