...
OFT_CHUNK_SIZE = 8
OFT_ACTION_DIM = 7
MAX_STEPS_PER_ROLLOUT = 600  # legacy default (overridden per-suite below)
# Official OpenVLA-OFT per-suite step budgets (= longest training demo + margin), matching
# openvla-oft/experiments/robot/libero/run_libero_eval.py. These are the protocol the OFT
# literature and the Jetson eval use; a flat 600 is non-standard and inflates short suites.
TASK_MAX_STEPS = {
    "libero_spatial": 220,
    "libero_object": 280,
    "libero_goal": 300,
...
def run_rollout(
    ...
    `max_steps` is the number of ACTIVE policy steps (after NUM_STEPS_WAIT settle steps),
    matching the OFT convention `while t < max_steps + num_steps_wait`.
    ...
    cap = max_steps + NUM_STEPS_WAIT  # total iterations: NUM_STEPS_WAIT settle + max_steps active
