"""
Fine-tune OpenVLA-OFT on all 4 LIBERO task suites — 2-CAMERA + PROPRIO variant
(the full OFT paper recipe: third-person agentview + wrist eye_in_hand + 8-dim
proprio token). The mono-cam script (01_train_openvla_oft.py) plateaued at
90.25%; the official recipe reaches ~97% — this variant targets 93+%.
...
ARCHITECTURE NOTE:
    ...
    NOT stored as self.action_head on the VLM. This script:
      ...
      2. Creates L1RegressionActionHead + ProprioProjector separately, attaches both
         to the model BEFORE DeepSpeed init so ZeRO-2 manages their parameters.
      3. Builds the multimodal sequence MANUALLY (the stock HF forward handles one
         image only):  [BOS | patches_agentview(256) | patches_wrist(256) |
         proprio_token(1) | text ...]
         mirroring modeling_prismatic.py's [BOS | patches | text] ordering.
      4. Action-token hidden states are gathered PER SAMPLE from each sequence's
         true (unpadded) end — the mono-cam script sliced a fixed [-57:-1] window,
         which lands on right-padding for every sample shorter than the batch max,
         silently corrupting part of each mixed-length batch's supervision.

CONVENTIONS (must match eval exactly):
    - BOTH cameras stored upside-down (OpenGL) in HDF5 → [::-1, ::-1] at train,
      and the same flip applied to agentview_image / robot0_eye_in_hand_image at eval.
    - proprio = [ee_pos(3), ee_ori(3)=quat2axisangle(robot0_eef_quat), gripper(2)]
      (verified against LIBERO/scripts/create_dataset.py) — at eval build it as
      np.concatenate([obs["robot0_eef_pos"],
                      T.quat2axisangle(obs["robot0_eef_quat"]),
                      obs["robot0_gripper_qpos"]])
      and normalize with the proprio_mean/std stored in action_stats.json.
...
class LIBERODataset(Dataset):
    ...
    def __init__(
        ...
        self.proprio_mean = torch.tensor(action_stats["proprio_mean"], dtype=torch.float32)
        self.proprio_std  = torch.tensor(action_stats["proprio_std"],  dtype=torch.float32)
    ...
    def __getitem__(self, idx):
        ...
        with h5py.File(hdf5_path, "r") as f:
            ...
            # Wrist camera — same OpenGL flip convention as agentview.
            wrist_np = np.ascontiguousarray(
                demo["obs"]["eye_in_hand_rgb"][t][::-1, ::-1]
            )  # (H, W, 3) uint8

            # Proprio: ee_pos(3) + ee_ori(3) + gripper_states(2) = 8D
            # ee_ori is quat2axisangle(robot0_eef_quat) per LIBERO create_dataset.py.
            ee_pos        = demo["obs"]["ee_pos"][t]           # (3,)
            ee_ori        = demo["obs"]["ee_ori"][t]           # (3,) axis-angle
        ...
        # ── Process images (both cameras, independent photometric augmentation) ─
        def _process(img_np):
            ...
            if self.augment:
                ...
                if random.random() > 0.5:
                    pil = TF.adjust_brightness(pil, brightness_factor=random.uniform(0.8, 1.2))
                if random.random() > 0.5:
                    pil = TF.adjust_contrast(pil, contrast_factor=random.uniform(0.8, 1.2))
            return self.processor.image_processor(
                images=pil, return_tensors="pt"
            )["pixel_values"].squeeze(0)

        pixel_values       = _process(image_np)
        pixel_values_wrist = _process(wrist_np)

        # Normalized proprio (same mean/std convention as actions)
        proprio_t = (torch.from_numpy(proprio) - self.proprio_mean) / (self.proprio_std + 1e-8)
...
def collate_fn(batch):
    ...
    return {
        ...
        "pixel_values_wrist": torch.stack([b["pixel_values_wrist"] for b in batch]),
...
def compute_action_stats(suite_names: list, data_root: str) -> dict:
    ...
    Also computes proprio stats (ee_pos + ee_ori + gripper, 8-dim) for the
    proprio-token normalization."""
        ...
        for hdf5_path in hdf5_list:
            with h5py.File(hdf5_path, "r") as f:
                for demo_key in f["data"].keys():
                    ...
                    all_proprio.append(np.concatenate([
                        demo["obs"]["ee_pos"][:],
                        demo["obs"]["ee_ori"][:],
    ...
    proprio = np.concatenate(all_proprio, axis=0)  # (N_total, 8)
    return {
        ...
        "proprio_std":  proprio.std(axis=0).tolist(),
    }


# ─────────────────────────────────────────────────────────────────────────────
# 2-cam + proprio multimodal forward
#
# The stock HF modeling_prismatic forward handles exactly one image, inserting
# patches as [BOS | patches | text...]. We extend that ordering to
# [BOS | patches_agentview | patches_wrist | proprio_token | text...] and call
# the language model directly with inputs_embeds. Because both train and eval
# use THIS function, train/eval consistency is guaranteed by construction.

def multimodal_forward(
    vla,                 # the underlying OpenVLAForActionPrediction module
    input_ids,           # (B, L)  text ids: [BOS, prompt..., action placeholders, (EOS)]
    attention_mask,      # (B, L)
    pixel_values,        # (B, 6, 224, 224)  agentview  (DINOv2+SigLIP fused channels)
    pixel_values_wrist,  # (B, 6, 224, 224)  wrist cam
    proprio,             # (B, 8)  normalized proprio
):
    """Returns (last_hidden, n_extra) where last_hidden is (B, S, D) and n_extra
    is the number of tokens inserted after BOS (patches x2 + proprio)."""
    input_embeddings = vla.get_input_embeddings()(input_ids)            # (B, L, D)

    patches_agent = vla.projector(vla.vision_backbone(pixel_values))        # (B, 256, D)
    patches_wrist = vla.projector(vla.vision_backbone(pixel_values_wrist))  # (B, 256, D)
    proprio_tok   = vla.proprio_projector(proprio).unsqueeze(1)             # (B, 1, D)
    ...
    mm_embeds = torch.cat([
        ...
        patches_wrist,
        proprio_tok,
        input_embeddings[:, 1:, :],       # prompt + action placeholders (+ EOS + pad)
    ], dim=1)
    ...
    return out.hidden_states[-1], n_extra


def gather_action_hidden(last_hidden, attention_mask, n_extra, action_len, has_eos: bool):
    """Per-sample gather of the action-token hidden states.

    With right padding, sample i's true multimodal length is
    S_i = attention_mask[i].sum() + n_extra. Action tokens are the action_len
    positions before EOS (training, has_eos=True) or the last action_len
    positions (inference, has_eos=False). A fixed [-57:-1] slice would read
    pad positions for any sample shorter than the batch max.
    """
    B = last_hidden.shape[0]
    lengths = attention_mask.sum(dim=1).long() + n_extra          # (B,)
    end = lengths - 1 if has_eos else lengths                     # exclusive end of action span
    offsets = torch.arange(action_len, device=last_hidden.device) # (action_len,)
    idx = (end.unsqueeze(1) - action_len) + offsets.unsqueeze(0)  # (B, action_len)
    batch_idx = torch.arange(B, device=last_hidden.device).unsqueeze(1)
    return last_hidden[batch_idx, idx]                            # (B, action_len, D)
...
def train(args):
    ...
    if stats_path.exists():
        ...
        # A stats file from the mono-cam run lacks proprio stats — recompute.
        stats_valid = "proprio_mean" in action_stats
        if is_main:
            print(f"[Data] Loaded action stats from {stats_path} "
                  f"(proprio stats {'present' if stats_valid else 'MISSING — recomputing'})")
    if not stats_valid:
        if is_main:
    ...
    # ProprioProjector — 8-dim proprio → one LLM token (OFT paper recipe).
    proprio_projector = ProprioProjector(
        llm_dim=llm_dim, proprio_dim=OFT_PROPRIO_DIM
    ).to(torch.bfloat16)
    model.proprio_projector = proprio_projector  # attach so DeepSpeed shards it
    ...
    for epoch in range(args.start_epoch, args.num_epochs):
        ...
        for batch_idx, batch in enumerate(
            ...
            pixel_values_wrist = batch["pixel_values_wrist"].to(device, dtype=torch.bfloat16)
            ...
            # 2-cam + proprio forward: [BOS | patches_agent | patches_wrist |
            # proprio | text...] — see multimodal_forward().
            last_hidden, n_extra = multimodal_forward(
                model_engine.module,
                input_ids, attention_mask,
                pixel_values, pixel_values_wrist, proprio,
            )

            # Per-sample gather of action-token hiddens from each sequence's true
            # end (has_eos=True: EOS is the final real token in training samples).
            action_len = args.chunk_size * OFT_ACTION_DIM
            action_hidden = gather_action_hidden(
                last_hidden, attention_mask, n_extra, action_len, has_eos=True
            )  # (B, action_len, D)

            # Predict actions via action_head: (B, action_len, D) → (B, chunk, 7)
        ...
        if is_main:
            ...
            try:
                ...
                torch.save(
                    {k: v.detach().cpu().clone()
                     for k, v in model_engine.module.proprio_projector.state_dict().items()},
                    ckpt_dir / "proprio_projector.pt",
                )
...
def _run_quick_eval(model_engine, processor, action_tokenizer, action_stats, args, epoch):
    ...
    # robosuite transform utils — same quat2axisangle used by LIBERO's
    # create_dataset.py, so eval proprio matches the HDF5 ee_ori convention.
    ...
    for suite_name in LIBERO_SUITES:
        ...
        # Placeholder action token IDs — identical to training input & 02B eval
        ...
        zero_chunk = np.zeros((args.chunk_size, OFT_ACTION_DIM), dtype=np.float32)
        disc = np.digitize(np.clip(zero_chunk, -1.0, 1.0), action_tokenizer.bins)
        ...
        for rollout_idx in range(3):
            ...
            while not done and step < 600:
                # Rotate 180° to match the training/eval image convention (both cams).
                ...
                pixel_values_wrist = processor.image_processor(
                    images=wrist, return_tensors="pt"
                )["pixel_values"].to(model_engine.device, dtype=torch.bfloat16)

                # Proprio: same construction + normalization as training.
                ...
                proprio_t = torch.from_numpy(proprio_np).unsqueeze(0).to(
                    model_engine.device, dtype=torch.bfloat16
                )

                with torch.no_grad():
                    # Same forward path as training — no EOS at inference, so the
                    # action tokens are the LAST action_len positions.
                    last_hidden, n_extra = multimodal_forward(
                        model_engine.module,
                        input_ids, attention_mask,
                    ...
                    action_hidden = gather_action_hidden(
                        last_hidden, attention_mask, n_extra, action_len, has_eos=False
                    )
                ...
                for ac_step in range(args.chunk_size):
                    if step >= 600 or done:
                        break
...
def parse_args():
    ...
    p.add_argument("--output_dir", default="checkpoints/openvla_oft_libero_2cam")
    p.add_argument("--run_name",   default="openvla_oft_libero_2cam_run1")
    ...
    p.add_argument("--chunk_size", type=int, default=OFT_CHUNK_SIZE,
                   help="Action chunk length K (default 8, the OFT LIBERO recipe). "
                        "The chunk-size × quantization study trains K ∈ {4, 8, 16} "
                        "variants; the action-token count is K * 7 per sample.")
